...
import { AnimatedSection, StaggerContainer, StaggerItem, scaleUp } from "./AnimatedSection";
  ...
  return (
    <section id="temoignages" className="section-padding bg-background">
      <div className="container-hotel">
        ...
        <StaggerContainer className="grid md:grid-cols-2 gap-8">
          {testimonials.map((testimonial) => (
            <StaggerItem key={testimonial.name}>
              <Card 
                className="bg-card border-border hover:border-primary/30 transition-all duration-300 hover:shadow-card h-full"
              >
        ...
        {/* Trust Indicators */}
        <AnimatedSection className="mt-16 text-center" variants={scaleUp}>
          <div className="inline-flex items-center gap-8 flex-wrap justify-center">
            {[
              { value: "500+", label: "Clients satisfaits" },
              { value: "4.8", label: "Note moyenne" },
              { value: "77", label: "Chambres disponibles" },
            ].map((stat, i) => (
              <motion.div
                key={stat.label}
